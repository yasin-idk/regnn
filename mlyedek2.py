...
class MLModels:
    ...
    def _display_results(self, results, input_cols, output_cols, df_processed, ml_models):
        ...
        result_tab1, result_tab2, result_tab3, result_tab4, result_tab5, result_tab6 = st.tabs(["📊 Model Comparison", "📐 Model Equations", "🎯 Make Predictions", "🧪 Variable Impact Test", "⚖️ Compare All Models", "🔬 Variable Effect Analysis"])
...
if uploaded_file is not None:
    try:
        ...
        with col1:
            ...
            if input_cols:
                st.success(f"Auto-detected {len(input_cols)} input columns")
            else:
                st.warning("No input columns auto-detected")
        with col2:
            st.write("**Output Columns:**")  
            if output_cols:
                st.success(f"Auto-detected {len(output_cols)} output columns")
            else:
                st.warning("No output columns auto-detected")
        ...
        # Quick validation and next steps info
        ...
        # Model Management Section (Main Focus)
        st.divider()
        ...
        data_tab1, data_tab2, data_tab3, data_tab4 = st.tabs(["📋 Raw Data", "🔍 Column Details", "📈 Statistics & Correlation", "📉 Relationships"])
        ...
        with data_tab4:
            if input_cols and output_cols:
                ...
                st.info("Select input and output columns to see relationships.")
